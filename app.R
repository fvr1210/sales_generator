...
ui <- dashboardPage(skin = "red",
                    ...
                    dashboardBody(tabItem(tabName = "generator",
                                          ...
                                          fluidRow( 
                                            ...
                                            column(4, numericInput(inputId="holiday_Neujahr", label = 'Weight New Year:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Heilige.Drei.Konige", label = 'Weight Epiphany/Heilige Drei Koenige:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Karfreitag", label = 'Weight Good Friday/Karfreitag:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Ostern", label = 'Weight Easter:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Ostermontag", label = 'Weight Easter Monday:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Erster.Mai", label = 'Weight May Day:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Christi.Himmelfahrt", label = 'Weight Ascension Day/Himmelfahrt:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Pfingsten", label = 'Weight Pentecost/Pfingsten:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Pfingstmontag", label = 'Weight Whit Monday/Pfingstmontag:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Fronleichnam", label = 'Weight Corpus Christi/Fronleichnam:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Tag.der.Deutschen.Einheit", label = 'Weight German unification/Deutschen Einheit:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Allerheiligen", label = 'Weight All Saints Day/Allerheiligen:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Erster.Weihnachtstag", label = 'Weight Christmas Day/Erster Weihnachtstag:',
                                                                   value=0)),
                                            column(4, numericInput(inputId="holiday_Zweiter.Weihnachtstag", label = 'Weight Boxing Day/Zweiter Weihnachtstag:',
                                                                   value=0))
                                            
                                          ), 
                                          # 
                                          fluidRow(
                                            tags$h4("Effects (do not change the Effect Name)"),
                                            numericInput(inputId = "n_DIF",label = "Number of Effects", 2),
...
server <- function(input, output, session) {
  ...
  output$dynamicRow <- renderUI({
    if (input$ouputfile == "BI_SALES") {
      fluidRow(
        tags$h4("For how many additional locations do you need the sales data (additional location name will be EXT_LOC_ID + 1, + 2 etc.)"),
        numericInput(inputId = "n_LOC_ID", label = "Number of additional Locations", value = 0),
      )
  ...
  # dynamic Effect input (was called DIF in previous version but do not create confusion renamed to Effect but DIF is still used for the variables) ----
  observeEvent(input$n_DIF, {
    ...
    output$DIF_START = renderUI({
      input_list <- lapply(1:input$n_DIF, function(i) {
        ...
        dateInput(DIF_Start, "Start of Effect:", value = Sys.Date()-729, format = "dd/mm/yyyy")
      })
      do.call(tagList, input_list)
    })
    ...
    output$DIF_END = renderUI({
      input_list <- lapply(1:input$n_DIF, function(i) {
        ...
        dateInput(DIF_End, "End of Effect:", value = Sys.Date(), format = "dd/mm/yyyy")
      })
      do.call(tagList, input_list)
    })
    ...
    output$DIF_NAME = renderUI({
      input_list <- lapply(1:input$n_DIF, function(i) {
        ...
        textInput(DIF_Name, "Effect Name", value = paste("DIF", i, sep = "_"))
      })
      do.call(tagList, input_list)
    })
    ...
    output$DIF_WEIGHT = renderUI({
      input_list <- lapply(1:input$n_DIF, function(i) {
        ...
        numericInput(DIF_Weight, "Effect Weight", 1)
      })
      do.call(tagList, input_list)
    })
